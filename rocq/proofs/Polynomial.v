...
(** Standard polynomial evaluation (little-endian coefficients). *)
...
(** poly_eval splits at the last element (append singleton). *)
...
(** poly_eval decomposes as removelast + last. *)
...
(** Horner chain: fold-left with multiply-and-add.
    horner_chain seed r [c1; c2; ...; cn] computes:
      (...((seed * r + c1) * r + c2) * r + ... + cn) *)
...
(** horner_chain relates to poly_eval on the reversed input. *)
...
(** ** PolyUHF correctness
    ...
    The circuit starts with seed = last(x) and feeds
    rev(removelast(x)) through the Horner chain.
    We prove this equals poly_eval x r. *)
...
(** ** PolyUHF base case (n = 2)
    Constraint: out = x[1] * r + x[0]
    This is exactly poly_eval [x0; x1] r. *)
