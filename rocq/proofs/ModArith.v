...
(** ** ModSum (mod.circom:7-11)
    Constraints (from TruncNumLE(n+1, n) applied to a + b):
      bits[i] * (bits[i] - 1) = 0   for i = 0..n
      a + b = bits_to_num bits       (length n+1)
...
(** ** ModSub (mod.circom:15-19)
    Constraints (from TruncNumLE(n+1, n) applied to a - b + 2^n):
      bits[i] * (bits[i] - 1) = 0   for i = 0..n
...
(** ** ModSumThree (mod.circom:22-27)
    Constraints (from TruncNumLE(n+2, n) applied to a + b + c): *)
...
(** ** ModSubThree (mod.circom:31-36)
    Constraints (from TruncNumLE(n+2, n) applied to a - b - c + 2^(n+1)): *)
...
(** ** ModSumFour (mod.circom:39-45)
    Constraints (from TruncNumLE(n+2, n) applied to a + b + c + d): *)
...
(** ** ModProd (mod.circom:49-56)
    Constraints:
      product = a * b   (multiplication constraint)
      TruncNumLE(2*n, n) applied to product *)
...
(** ** Split (mod.circom:60-78)
    Constraints:
      bits[i] * (bits[i] - 1) = 0   for i = 0..n+m-1
      in = bits_to_num bits          (length n + m)
      small = bits_to_num (firstn n bits)
      big = bits_to_num (skipn n bits)

    We prove: in = small + big * 2^n with range checks. *)
...
(** ** SplitThree (mod.circom:82-107)
    Constraints:
      bits[i] * (bits[i] - 1) = 0   for i = 0..n+m+k-1
      in = bits_to_num bits          (length n + m + k)
      small = bits_to_num (firstn n bits)
      medium = bits_to_num (firstn m (skipn n bits))
      big = bits_to_num (skipn (n + m) bits)

    We prove: in = small + medium * 2^n + big * 2^(n+m) with range checks. *)
