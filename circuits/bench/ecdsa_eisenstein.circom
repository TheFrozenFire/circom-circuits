...
include "ecdsa/eisenstein_glv.circom";
include "ecdsa/scalarmul.circom";

/// ECDSA verification using Eisenstein integer scalar multiplication (68-bit MSM loop).
/// Benchmark variant — same algorithm as ECDSAVerifyNoPubkeyCheck but uses
/// Secp256k1EisensteinScalarMult instead of other scalar mult variants for u2·pubkey.
/// The prover must supply u2pub_hint = [u2]·pubkey externally.
template ECDSAVerifyEisenstein(n, k) {
    assert(n == 32 && k == 8);
    ...
    // Step 5: u2 * pubkey (Eisenstein — 68-bit MSM loop)
    component u2Pub = Secp256k1EisensteinScalarMult(n, k);
    for (var i = 0; i < k; i++) {
